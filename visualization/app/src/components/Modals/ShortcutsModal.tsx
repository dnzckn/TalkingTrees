...
export function ShortcutsModal({ onClose }: { onClose: () => void }) {
  return (
    <div style={{
      position: 'fixed', inset: 0, background: 'var(--s-overlay)',
      ...
      <div style={{
        ...
        <div style={{
          padding: '14px 20px', borderBottom: '1px solid var(--b0)',
        ...
        <div style={{ padding: '12px 20px', overflow: 'auto', maxHeight: '60vh' }}>
          {SHORTCUTS.map(s => (
            <div key={s.section} style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 'var(--fs-xs)', fontWeight: 600, color: 'var(--t2)', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6 }}>
                {s.section}
              </div>
              {s.items.map(i => (
                <div key={i.keys} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0', fontSize: 'var(--fs-sm)' }}>
                  <span style={{ color: 'var(--t1)' }}>{i.desc}</span>
                  <kbd style={{
                    background: 'var(--s0)', border: '1px solid var(--b1)', borderRadius: 3,
                    padding: '2px 8px', fontFamily: 'var(--mono)', fontSize: 'var(--fs-xs)', color: 'var(--t2)',
                  }}>{i.keys}</kbd>
                </div>
              ))}
            </div>
