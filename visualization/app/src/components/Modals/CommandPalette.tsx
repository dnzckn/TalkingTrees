...
export function CommandPalette({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'var(--surface-overlay)', zIndex: 10000, display: 'flex', justifyContent: 'center', paddingTop: '15vh' }}
      onClick={onClose}
    >
      <div style={{
        width: 480, maxHeight: '50vh', background: 'var(--surface-2)', border: '1px solid var(--border-1)',
        ...
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-0)' }}>
          <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)} onKeyDown={handleKey}
            placeholder="Type a command..." style={{ width: '100%', padding: '8px 12px', fontSize: 'var(--fs-lg)', background: 'var(--surface-0)', border: '1px solid var(--border-1)', borderRadius: 'var(--r-md)' }}
          />
        </div>
        {/* Results */}
        <div style={{ flex: 1, overflow: 'auto', padding: '4px 0' }}>
          {filtered.map((cmd, i) => (
            ...
            >
              <span style={{ color: 'var(--text-2)', fontSize: 'var(--fs-xs)', fontWeight: 600, width: 60, textTransform: 'uppercase' }}>{cmd.section}</span>
              <span style={{ flex: 1, color: 'var(--text-0)' }}>{cmd.label}</span>
              {cmd.shortcut && <kbd style={{
                background: 'var(--surface-0)', border: '1px solid var(--border-1)', borderRadius: 3,
                padding: '2px 7px', fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-xs)', color: 'var(--text-2)',
              }}>{cmd.shortcut}</kbd>}
            </div>
          ))}
          {filtered.length === 0 && <div style={{ padding: 16, textAlign: 'center', color: 'var(--text-3)' }}>No matching commands</div>}
        </div>
      </div>
    </div>
  );
