...
class TreeMerger:
    ...
    def merge_trees(
        ...
        merged = self._apply_merge(base, our_diff, their_diff, conflicts, strategy)
    ...
    def _apply_merge(
        ...
        For now, returns 'ours' or 'theirs' based on strategy.
        Full merge implementation would need to apply individual changes.
        """
        # Simplified: just return one side based on strategy
        # Full implementation would need to:
        # 1. Clone base tree
        # 2. Apply non-conflicting changes from both sides
        # 3. Apply conflicting changes based on strategy

        if strategy == "ours":
            # For now, return ours (would need proper merge logic)
